            ...
            RTCP.forEach(event.rtcpPacket.packetData,
                ...
                {payloadType: RTCP.PT_RTPFB, feedbackMessageType: RTCP.FMT_ALFB, filter: (decoded, view) => {
                    ...
                    if (event.rtcpPacket.incoming) {
                        // Note receive times.
                        let total = 0;
                        const totalDelta = result.delta.forEach((delta, i) => {
                            total += delta;
                            receiveTimes.push([result.baseSequenceNumber + i, result.referenceTime_ms + total / 1000]);
                        });
                        if (lastSequenceNumber) {
                            const Ti = sendTimes.find(item => item[0] === result.baseSequenceNumber + result.delta.length - 2)[1];
                            const Ti_1 = sendTimes.find(item => item[0] === lastSequenceNumber)[1];
                            const ti = receiveTimes.find(item => item[0] === result.baseSequenceNumber + result.delta.length - 2)[1];
                            const ti_1 = receiveTimes.find(item => item[0] === lastSequenceNumber)[1];
                            d_i.push([Ti, ti - ti_1 - (Ti - Ti_1)]);
                        }
